'use strict';
angular.module('conFusion.services', ['ngResource']).constant('baseURL', 'https://psycolapp.herokuapp.com/').factory('headerFactory', ['$resource', 'baseURL', function($resource, baseURL) {
    return $resource(baseURL + 'phrases/', null, { 'update': { method: 'PUT' } })
}]).factory('MarketFactory', ['$resource', 'baseURL', function($resource, baseURL) {
    return $resource(baseURL + 'market/:id', { id: '@_id' }, { 'update': { method: 'PUT' } })
}]).factory('BlogFactory', ['$resource', 'baseURL', function($resource, baseURL) {
    return $resource(baseURL + 'blog/:id', { id: '@_id' }, { 'update': { method: 'PUT' } })
}]).factory('orderFactory', ['$resource', 'baseURL', function($resource, baseURL) {
    return $resource(baseURL + 'checkout/', null, { 'update': { method: 'PUT' } })
}]).factory('commentFactory', ['$resource', 'baseURL', function($resource, baseURL) {
    return $resource(baseURL + 'blog/:id/comments/:commentId', { id: '@_id', commentId: '@_commentId' }, { 'update': { method: 'PUT' } })
}]).factory('PostFactory', ['$http', function($http) {
    var o = { posts: [] };
    o.upvote = function(post) {
        return $http.put('/blog/' + post._id + '/upvote', null, {}).success(function(data) {
            console.log(post);
            post.upvotes[0].value += 1;
        });
    };
    o.downvote = function(post) {
        return $http.put('/blog/' + post._id + '/downvote', null, {}).success(function(data) { post.downvotes[0].value -= 1; });
    };
    o.upvoteComment = function(post, comment) {
        return $http.put('/blog/' + post._id + '/comments/' + comment._id + '/upvote', null, {}).success(function(data) { comment.upvotes[0].value += 1; });
    };
    o.downvoteComment = function(post, comment) {
        return $http.put('/blog/' + post._id + '/comments/' + comment._id + '/downvote', null, {}).success(function(data) { comment.downvotes[0].value -= 1; });
    };
    o.get = function(id) {
    ...
    function loadUserCredentials() {
        ...
        if (credentials.username != undefined) { useCredentials(credentials); }
    }
    ...
    authFac.login = function(loginData) {
        $resource(baseURL + "users/login").save(loginData, function(response) {
            ...
            var message = '\
                ...
                <div><h3>Login Unsuccessful</h3></div>' + '<div><p>' + response.data.err.message + '</p><p>' + response.data.err.name + '</p></div>' + '<div class="ngdialog-buttons">\
    ...
    authFac.register = function(registerData) {
        $resource(baseURL + "users/register").save(registerData, function(response) {
            authFac.login({ username: registerData.username, password: registerData.password });
            if (registerData.rememberMe) { $localStorage.storeObject('userinfo', { username: registerData.username, password: registerData.password }); }
            $rootScope.$broadcast('registration:Successful');
        }, function(response) {
            var message = '\
                <div class="ngdialog-message">\
                <div><h3>Registration Unsuccessful</h3></div>' + '<div><p>' + response.data.err.message + '</p><p>' + response.data.err.name + '</p></div>';
